...
## 作用
- 管理员查询卡信息，确认后注销该卡，展示退款明细（押金 + 余额合计）
...
## 预计退款计算
- 前端本地计算：`(cardInfo.balance + cardInfo.deposit) / 100` 元，仅作预览显示
...
## 状态变化
```
cardId 变化 → 清空 cardInfo / result / error
查询成功 → cardInfo 填入
...
- `POST /api/cards/{id}/cancellation`（cancelCard）—— 注销

## 备注 / 待确认
- 使用 `Modal.confirm` 实现二次确认，防止误操作
- 注销成功后清空 cardInfo，避免用户对已注销卡重复操作
- 退款明细中「应退合计」用绿色加粗字体（#52c41a，fontSize 16）突出显示
