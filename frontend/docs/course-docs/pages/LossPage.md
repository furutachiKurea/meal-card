...
## 作用
- 管理员查询持卡人当前有效卡状态，对正常卡申请挂失，或对已挂失卡取消挂失
...
## 状态变化
```
输入值变化 → 清空 cardInfo / successMsg / error
...
- `PUT /api/cards/{cardNo}/loss-report`（reportLoss）—— 挂失（路径参数为 16 位 cardNo）
- `DELETE /api/cards/{cardNo}/loss-report`（cancelLossReport）—— 取消挂失

## 备注 / 待确认
- 挂失/取消挂失后，cardInfo 直接用接口返回值替换，不需要重新查询
- 后端按证件号查询只返回 active 或 lost 状态的最新卡，cancelled 卡不会出现在结果中
